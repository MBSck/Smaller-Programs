...
"""Same as before just switched the softmax function with the sigmoid function:
sigmoid(f_i)=1/(1+exp(-f_i))"""
...
# Stochastische Mittelung nicht so erfolgreich mit gegebenen Einstellungen, bei Teilen geht es gut
# computes the mean
...
def setup_loss_function(X_train, Y_train, batch_size=100):
    ...
    def loss():
        ...
        return tf.reduce_sum(
            (tf.nn.sigmoid(tf.matmul(X, W)+B)-Y)**2
        )
...
# output for batch_size = 5000: 46,61%
# same accuracy as there are not more than two classes used. If more than two classes used the weights are not
